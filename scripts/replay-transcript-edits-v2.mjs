...
// Seed from current disk content when available
for (const rel of targets.size ? targets : new Set(ops.map((o) => o.rel))) {
  const disk = path.join(projectRoot, rel);
  if (fs.existsSync(disk)) {
    files.set(rel, fs.readFileSync(disk, "utf8"));
  }
}

for (const op of ops) {
  if (op.kind === "Write") {
    // Only use Write if we have no content yet for this file
    if (!files.has(op.rel)) {
