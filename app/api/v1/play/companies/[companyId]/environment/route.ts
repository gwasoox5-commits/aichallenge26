...
export async function GET(req: Request, { params }: { params: Promise<{ companyId: string }> }) {
  try {
    ...
    const company = await engine.getCompany(companyId);
    const env = await engine.getCeoEnvironment(company.sessionId);
...
export async function POST(req: Request, { params }: { params: Promise<{ companyId: string }> }) {
  try {
    ...
    await assertCompanyAccess(ctx, engine, companyId);
    const company = await engine.getCompany(companyId);
    const result = await engine.acknowledgeCeoEnvironmentBadge(company.sessionId);
